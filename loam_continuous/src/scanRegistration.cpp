...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn2)
{
  ...
  if (imuPointerLast >= 0)
  {
    ...
    else
    {
      ...
      float ratioBack = (imuTime[imuPointerFront] - timeScanCur) / (imuTime[imuPointerFront] - imuTime[imuPointerBack]);
  ...
  int startPoints[4] = { 5, 6 + int((cloudSize - 10) / 4.0), 6 + int((cloudSize - 10) / 2.0),
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    for (int j = ep; j >= sp; j--)
    {
      if (cloudNeighborPicked[cloudSortInd[j]] == 0 && laserCloud->points[cloudSortInd[j]].s > 0.1 &&
          ...
          fabs(laserCloud->points[cloudSortInd[j]].x) < 30 && fabs(laserCloud->points[cloudSortInd[j]].y) < 30 &&
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/cloud_in", 2, laserCloudHandler);

  ros::Subscriber subImu = nh.subscribe<sensor_msgs::Imu>("/imu", 5, imuHandler);

...
void transformAssociateToMap()
{
  ...
  transformTobeMapped[3] =
      transformAftMapped[3] - (cos(transformTobeMapped[1]) * x2 + sin(transformTobeMapped[1]) * z2);
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subLaserCloudLast2 =
      nh.subscribe<sensor_msgs::PointCloud2>("/laser_cloud_last_2", 2, laserCloudLastHandler);

  ros::Subscriber subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("/cam_to_init", 5, laserOdometryHandler);
  ...
  while (status)
  {
    ...
    if (newLaserCloudLast && newLaserOdometry && fabs(timeLaserCloudLast - timeLaserOdometry) < 0.005)
    {
      ...
      if (centerCubeI < 0 || centerCubeI >= laserCloudWidth || centerCubeJ < 0 || centerCubeJ >= laserCloudHeight ||
              ...
              for (int ii = -1; ii <= 1; ii += 2)
              {
                for (int jj = -1; jj <= 1; jj += 2)
                {
                  for (int kk = -1; kk <= 1; kk += 2)
                  {
                    ...
                    float check = 100.0 + (transformTobeMapped[3] - cornerX) * (transformTobeMapped[3] - cornerX) +
      ...
      {
        if (fabs(laserCloudFromMap->points[i].v - 2) < 0.005 || fabs(laserCloudFromMap->points[i].v - 1) < 0.005)
        {
      ...
      *laserCloudLast = *laserCloudCorner2 + *laserCloudSurf2;
      ...
      if (laserCloudSurfFromMap->points.size() > 500)
      {
        ...
        for (int iterCount = 0; iterCount < 10; iterCount++)
        {
          ...
          if (fabs(matX.at<float>(0, 0)) < 0.5 && fabs(matX.at<float>(1, 0)) < 0.5 &&
              fabs(matX.at<float>(2, 0)) < 0.5 && fabs(matX.at<float>(3, 0)) < 1 && fabs(matX.at<float>(4, 0)) < 1 &&
      ...
      geoQuat = tf::createQuaternionMsgFromRollPitchYaw(transformAftMapped[2], -transformAftMapped[0],
